...
<head>
    ...
    <title>Listening Test - Academic</title>
    ...
    <div class="navbar-container fixed-top">
        ...
        <nav class="navbar navbar-expand-lg navbar-light nav-bar-2">
            ...
            <div class="container">
                ...
                <div></div>
                @foreach ($organizedData as $row)
                    <iframe src="{{ asset($row['audio_file']) }}" class="audio-play" id="audio-player" allow="autoplay"
                                                    ...
                                                    @if (!empty($questioninfo))
                                                        ...
                                                        @foreach ($questioninfo as $questionId => $question)
                                                            @if ($q_count + 1 >= $part['start'] && $q_count + 1 <= $part['end'])
                                                                <div class="test-panel__item mb-2"
                                                                    id="@php
                            ...
                            @foreach ($partInfo as $part)
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link {{ $loop->first ? 'active' : '' }} d-flex"
                                        ...
                                        data-bs-target="#pills-part{{ $part['id'] + 1 }}" type="button"
                                        role="tab" aria-controls="pills-part{{ $part['id'] + 1 }}"
                                        ...
                                        <div class="tab-number-count">
                                            @for ($i = $part['start']; $i <= $part['end']; $i++)
                                                <span onclick="scrollToQues('ques-{{ $i }}', event)">
                                                    <p>{{ $i }}</p>
                                                </span>
                                            @endfor
                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function submitForm() {
            ...
            $.ajax({
                url: "{{ route('academic-listening-test-score') }}",
                type: 'POST',
                data: {
                ...
                success: function(response) {
                    window.location.href = '/academic/reading/confirm_details';
                },
